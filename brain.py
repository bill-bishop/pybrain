...
# --- stubbed intelligence ---
def generate_program(history_text: str, observations_text: str, current_input: str) -> str:
...
class RunnerConfig:
    ...
    session_root: Path = Path("/work/.runner_session")
...
def apply_observation_side_effects(cfg: RunnerConfig, obs: Observations, fields: Dict[str, List[str]]) -> None:
    ...
    for f in fields.get("OPENFILE", []):
        p = Path(f)
        if not p.exists() or not p.is_file():
            obs.open_files[f] = FileSnapshot(f, ["(missing)"], [], 0, "")
            continue
        raw = p.read_bytes()
        ...
        obs.open_files[f] = FileSnapshot(f, head, tail, trunc, sha)

    for f in fields.get("CLOSEFILE", []):
        obs.open_files.pop(f, None)
...
def execute_script(cfg: RunnerConfig, script_text: str, run_dir: Path) -> Tuple[bool,int,int,str,str]:
    ...
    return ok, proc.returncode, dt_ms, stdout_text, stderr_text
...
class Session:
    ...
    def submit(self, user_input: str) -> RunRecord:
        """
        One user turn. Internally may perform multiple auto-Continue runs
        if STEPS_REMAINING > 0. Returns the *last* RunRecord produced in this turn.
        """
        ...
        while True:
            ...
            # The model sees: History, then Observations, then Input.
            script = generate_program(history_text, observations_text, current_input)
...
def main():
    ...
    # interactive: read lines until EOF
    print("Enter inputs. Ctrl-D to exit.")
    for line in sys.stdin:
        line = line.rstrip("\n")
        if not line:
            continue
        rec = s.submit(line)
        # show the full envelope after each user message
